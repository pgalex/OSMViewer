...
public class MapSectorTest
{
	...
	public void loadingWithNullDataSourceTest() throws FetchingErrorException
	{
		try
		{
			...
			sector.loadObjects(null, new TestRenderableMapObjectsDrawSettingsFinder(), new TestMapLoadingHandler());
			fail();
		}
		catch (IllegalArgumentException ex)
		{
	 ...
	 */
	@Test
	public void loadingWithObjectsInAreaTest() throws FetchingErrorException, InterruptedException
	{
		...
		TestMapLoadingHandler loadingHandler = new TestMapLoadingHandler();
		sector.loadObjects(testMapDataSource, new TestRenderableMapObjectsDrawSettingsFinder(), loadingHandler);
		
		Thread.sleep(100);
		
		assertTrue(loadingHandler.mapLoadedCallsCount.get() > 0);
